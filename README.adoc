...
. For installation on WebSphere Liberty Core ensure that the application war file is extracted to your server's apps directory, Servlet 3.0 is enabled as a feature in server.xml and a webApplication tag referencing Spriacle is in server.xml.
