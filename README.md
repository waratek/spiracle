...
The multi-stage `Dockerfile` builds the WAR with JDK 8 / Maven (`-Dversion.jdk=1.5 -Dversion.webxml=30`) and deploys it on Tomcat 9. `docker/entrypoint.sh` rewrites `conf/Spiracle.properties` from environment variables at startup.

> **Note — JDBC drivers:** This branch does not bundle any JDBC driver in the WAR. The Docker image places the drivers in Tomcat's `lib/`: MySQL Connector/J 5.1.49 (legacy `com.mysql.jdbc.Driver` classname), MSSQL `mssql-jdbc` 12.4.2.jre8, and Oracle `ojdbc8` 21.13. The MySQL stack runs `mysql:8.0` (`--default-authentication-plugin=mysql_native_password`, which Connector/J 5.1.49 can negotiate).
...
- Java 8 toolchain (compiles `-source 1.5 -target 1.5`; this branch targets Java 5 source level)
- Apache Maven
...
- **`-Dversion.jdk=<level>`** — Sets the Java source and target compiler level. On this branch the only supported value is `1.5`.
...
```sh
# Java 5, Servlet 3.0 (recommended)
$ mvn install -Dversion.jdk=1.5 -Dversion.webxml=30
```
...
This branch builds under a Java 8 toolchain with `-source 1.5 -target 1.5`. The Docker image pins its own JDK via its base image (`maven:3.9-eclipse-temurin-8`) — no local JDK is needed when building through Docker.

### Branch model

- **`java5` (this branch)** — Java 5 source-compatible variant. Same source tree as `master`, but the build targets the Java 5 source level only (`-Dversion.jdk=1.5`). Switch to `master` if you need a higher source level.
