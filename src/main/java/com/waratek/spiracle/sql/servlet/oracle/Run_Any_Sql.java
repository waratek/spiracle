...
public class Run_Any_Sql extends HttpServlet
{
    ...
    private String[] getSqlArgs(HttpServletRequest request) throws UnsupportedEncodingException
    {
        ...
        else
        {
            ...
            sqlArgs = (String) ParameterNullFix.sanitizeNull(queryStringList, request).get(ARGS); //take args from URL param if args cookie doesn't exist
        }
    ...
    private void executeRequest(HttpServletRequest request, HttpServletResponse response) throws IOException
    {
        ...
        String unformattedSql = (String) nullSanitizedMap.get(SQL);
        unformattedSql = DataSourceUtil.makeStringUntainted(unformattedSql);
        final String argSources = (String) nullSanitizedMap.get(ARG_SOURCES);
        String[] sqlArgs = getSqlArgs(request);
        sqlArgs = setArgDataSource(sqlArgs, argSources, request);

        String sql = unformattedSql;
        for (int i = 0; i < sqlArgs.length; i++) {
            int idx = sql.indexOf("%s");
            if (idx >= 0) {
                sql = sql.substring(0, idx) + sqlArgs[i] + sql.substring(idx + 2);
            }
        }
